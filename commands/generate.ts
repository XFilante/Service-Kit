...
import { ServiceKitConfig } from '../src/types.js'
...
export default class SchemaGenerate extends BaseCommand {
  ...
  #config = () => {
    const rawConfig = this.app.config.get<ServiceKitConfig | null>('service_kit', null)

    if (!rawConfig) {
      throw new RuntimeException(
        'Invalid "config/service_kit.ts" file. Make sure you are using the "defineConfig" method'
      )
    }
